...
extension ScrollPageView {
   ...
   @ViewBuilder
    func arcPage(arc: ArcModel) -> some View {
     ...
     func commentsForArcCount(_ arc: ArcModel) -> Int {
     ...
     func toggleSpoiler(for arcID: UUID) {
     ...
     func handleArcLike(_ arc: ArcModel) {
        do {
    ...
    @ViewBuilder
     ...
     func handleArcRepost(arcID: UUID) {
        do {
     ...
     func filteredArcs(for mode: ScrollMode) -> [ArcModel] {
        switch mode {
        case .feed:
            let base = dataStore.arcs.filter {
                $0.contentType == type &&
