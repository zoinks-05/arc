        ...
        ZStack{
            ...
            VStack {
                    ...
                    HStack(spacing: 8) {
                        ForEach(["feels", "believes", "thinks"], id: \.self) { option in
                            ...
                            Text(option)
                                ...
                                .padding(.vertical, 6)
                                .glassEffect(isSelected ? .regular.tint(.white) : .regular, in: Capsule())
                ...
                Button {
                    ...
                    // Optional: reset fields after publishing
                    ReflectionText = ""
                    DescriptionText = ""
                    ...
                    Text("Publish")
                        ...
                        .background(
                            ReflectionText.trimmingCharacters(in: .whitespacesAndNewlines).count >= 50
                            ? AnyShapeStyle(
                                LinearGradient(
                                    colors: [.purple, .red],
                                    startPoint: .leading,
                ...
                .buttonStyle(.plain)
                .disabled(ReflectionText.trimmingCharacters(in: .whitespacesAndNewlines).count < 50)
                .animation(.easeInOut, value: ReflectionText.trimmingCharacters(in: .whitespacesAndNewlines).count >= 50)
    ...
    private func loadData() async {
        do {
            ...
            await MainActor.run {
                ...
                title = res[contentType == "movie" ? "title" : "name"] as? String
                let genreObjects = res["genres"] as? [[String: Any]] ?? []
                availableThemes = genreObjects.compactMap { $0["name"] as? String}
            }
        } catch {
